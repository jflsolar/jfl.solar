...
const LoadProfiling = ({ loads, onSubmit, onBack}: Props) => {
  ...
  const [newAppliance, setNewAppliance] = useState<LoadProfile>({ name: '', quantity: 1, hoursPerDay: 1, powerRating: 100 });
  ...
  const totalDailyConsumption = appliances.reduce((total, appliance) => {
    return total + (appliance.quantity * appliance.hoursPerDay * appliance.powerRating) / 1000;
  }, 0);
  ...
  const handleSubmit = () => {
    ...
    setNewAppliance({ name: '', quantity: 1, hoursPerDay: 1, powerRating: 100 });
...
const handleNext = () => {
  if(appliances.length > 0) {
    ...
    toast({ title: "Empty Profile" ,variant: 'destructive', description: 'Please add atleast one item'})
  }
}

  return (
    <div className="flex flex-col gap-16 items-center w-full px-12">
      <h2 className="text-3xl font-bold text-center mb-8">Input Your Load Profile</h2>
     <div className='flex flex-col items-end gap-4 w-full'>

     <CustomButton text='Add Appliance/Device' variant='secondary' onClick={openAddDialog}/>
     ...
     <Table>
        <TableHeader>
          <TableRow>
            ...
            <TableHead className="w-[100px] text-center">Hours of Use per Day</TableHead>
            <TableHead className="text-right">Power Rating (Watt)</TableHead>
            <TableHead className="text-right">Ave. Daily Consumption</TableHead>
        ...
        <TableBody>
          {appliances.map((appliance, index) => (
            <TableRow key={index}>
              ...
              <TableCell className="text-center">{appliance.quantity}</TableCell>
              <TableCell className="text-center">{appliance.hoursPerDay}</TableCell>
              <TableCell className="text-right">{appliance.powerRating} W</TableCell>
              <TableCell className="text-right">
                {((appliance.quantity * appliance.hoursPerDay * appliance.powerRating) / 1000).toFixed(2)} kWh
              </TableCell>
              <TableCell className="text-right space-x-1">
                <Button variant="secondary" size="sm" onClick={() => editAppliance(index)}>
                  <Edit size={18}/>
                </Button>
                <Button variant="destructive" size="sm" onClick={() => deleteAppliance(index)}>
                  <Trash size={18}/>
                </Button>
              </TableCell>
            </TableRow>
      ...
      <div className="flex flex-col text-lg font-bold mt-4">
        <span>Total Daily Consumption: {totalDailyConsumption.toFixed(2)} kWh</span>
    ...
    <div className='flex justify-between w-full gap-4 items-center'>
      ...
      <CustomButton text='Next' variant='primary' onClick={() => handleNext()}/>
    </div>
    <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
      <DialogContent>
        <DialogHeader>
        <DialogTitle>{editIndex !== null ? 'Edit' : 'Add'} Appliance/Device</DialogTitle>
          <DialogDescription>
            {editIndex !== null ? 'Edit the appliance/device details.' : 'Fill out the details below to add a new appliance or device to your load profile.'}
          </DialogDescription>
        </DialogHeader>
        <div className="flex flex-col gap-4">
          ...
          <Button onClick={handleSubmit}>{editIndex !== null ? 'Save Changes' : 'Add Appliance'}</Button>
          </div>
      </DialogContent>
    </Dialog>
  </div>
